...
\begin{table}[]
    ...
    sentiment &
      ...
      triangle \\ \hline
    positive &
      fast, only little tiring, percice, fun &
      easy to do, known gesture, natural &
      predictable, easy to aim, uses two hands, kreative \\ \hline
    negative &
      produces the most false positive activations, index finger tipps down to meet the thumb while acivating, most suceptable to tracking errors for example when turning hand, unpredictable, conflicted with gesture to grab flask &
      tiring, puts pressure on wrist, difficult to aim very far, very near and persice since closing the hand can change the target and it is not easy to hold the hand paralell to the body &
      difficult to aim far, 2 handed use, some activation problems \\ \hline
    \end{tabular}%
    }
\label{tbl:interview}
\end{table}
