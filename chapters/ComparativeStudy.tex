...
\begin{table}[]
    ...
    sentiment &
      ...
      triangle \\ \hline
    positive &
        \makecell{fast, only little tiring, percice, fun} &
        \makecell{easy to do, known gesture, natural} &
        \makecell{predictable, easy to aim, uses two hands, kreative} \\ \hline
    negative &
      \makecell{produces the most false positive activations,\\ index finger tips down to meet the thumb while acivating,\\ most suceptable to tracking errors \\for example when turning hand,\\ unpredictable,\\ conflicted with gesture to grab flask} &
      \makecell{tiring,\\ puts pressure on wrist,\\ difficult to aim very far, very near and persice\\ since closing the hand can change the target \\and it is not easy to hold the hand paralell to the body} &
