...
The internal test game studios are using are not known, so the sources for testing methodology are limited. In the literature many studies rely on completion time and the number of errors during a task as a measure of a good locomotion system. %TOOO: cite
...
The testing environments used for the studies are very diverse. Sometimes the prototype implementations are using a detailed environment so that it is very immersive for the test participant and other times it is created so that its not distracting the player and therefore very clean and simple. %TODO: cite long path paper and 
